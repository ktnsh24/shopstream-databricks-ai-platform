...
# COMMAND ----------
# MAGIC %md
# MAGIC ## Step 5: Write model code to a file for code-based logging
# MAGIC
# MAGIC MLflow cannot pickle a class defined in a notebook scope.
# MAGIC Code-based logging writes the model class to a `.py` file and passes
# MAGIC the file path to `log_model` — MLflow then stores the source file itself.
...
class _QueryMetricsTool:
    ...
    _SQL = {
        "revenue": "SELECT CAST(order_date AS STRING) AS day, ROUND(SUM(total_revenue),2) AS revenue_eur FROM helix_databricks.default.revenue_daily WHERE order_date >= {start} GROUP BY 1 ORDER BY 1 DESC LIMIT {limit}",
        "orders": "SELECT CAST(order_date AS STRING) AS day, COUNT(*) AS orders FROM helix_databricks.default.fct_orders WHERE order_date >= {start} GROUP BY 1 ORDER BY 1 DESC LIMIT {limit}",
        "customers": "SELECT churn_risk_segment, COUNT(*) AS count, ROUND(AVG(predicted_churn_prob),3) AS avg_churn FROM helix_databricks.default.customer_metrics GROUP BY 1 ORDER BY 2 DESC LIMIT {limit}",
        "products": "SELECT product_name, ROUND(SUM(total_revenue),2) AS revenue FROM helix_databricks.default.product_performance WHERE snapshot_date >= {start} GROUP BY 1 ORDER BY 2 DESC LIMIT {limit}",
        "returns": "SELECT CAST(return_date AS STRING) AS day, COUNT(*) AS returns FROM helix_databricks.default.fct_returns WHERE return_date >= {start} GROUP BY 1 ORDER BY 1 DESC LIMIT {limit}",
    }
...
class _SearchDocumentsTool:
    name = "search_documents"
    description = "Semantic search over ShopStream policies and FAQs."
    parameters = {
        "type": "object",
        "properties": {
            "query": {"type": "string"},
...
class _ForecastTool:
    name = "forecast"
    description = "Predict future revenue using the @champion LightGBM model."
    parameters = {
        "type": "object",
...
_GATEKEEPER_SYSTEM = """You are a safety classifier for ShopStream AI.
Only allow questions about: revenue, orders, products, customers, return policies, shipping, warranties, forecasts, alerts.
Respond with EXACTLY: ALLOWED  or  BLOCKED: <reason>"""

_AGENT_SYSTEM = """You are ShopStream\\'s AI data assistant.
Use the available tools to answer questions about e-commerce data.
Think step by step. Call tools one at a time. Give a clear final answer."""

_TOOLS_REGISTRY: dict[str, Any] = {}
for _t in [_QueryMetricsTool(), _SearchDocumentsTool(), _ForecastTool(), _AlertTool()]:
    _TOOLS_REGISTRY[_t.name] = _t
...
def _run_agent(question: str, client: OpenAI) -> str:
    ...
    for _ in range(MAX_TOOL_ROUNDS):
        response = client.chat.completions.create(
            model=MAIN_AGENT_MODEL, messages=messages, tools=_tool_specs(), tool_choice="auto", temperature=0,
        )
...
with open(_MODEL_FILE, "w") as f:
    f.write(_MODEL_CODE)
print(f"Model code written to {_MODEL_FILE}")
